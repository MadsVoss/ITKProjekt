    ...
    public void saveLogin(final LinkedList<DTO> logintobatch) {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    PreparedStatement preparedStatement = conn.prepareStatement("INSERT INTO loginoplysninger(mail, password, cpr) VALUES (?, ?, ?)");
                    for (DTO dbDTO: logintobatch) {
                        preparedStatement.setString(1, dbDTO.getEmail());
                        preparedStatement.setString(2, dbDTO.getCpr());
                        preparedStatement.setString(3, dbDTO.getPassword());
...
//It has a limit of 150, this is because it would otherwise look cluttered. In reality it would have been nice to have the statement also select a given data and time to show some more specific data.
    ...
    public LinkedList<DTO> loadLogin(String id) {
        ...
        try {
            PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM loginoplysninger WHERE cpr = ? limit 150 ");
            preparedStatement.setString(1, id);
            ResultSet resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
                DTO nDTO = new DTO();
                nDTO.setId(Integer.parseInt(resultSet.getString("id")));
                nDTO.setEmail(resultSet.getString("email"));
                nDTO.setCpr(id);
